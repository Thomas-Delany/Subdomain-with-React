...
const CoursesDisplay = () => {
  const progressingCourses = courseData.slice(5, 6);
  ...
  const renderCoursesSection = (title, courses) => (
    <>
      ...
      <div className="grid grid-cols-1 place-items-center gap-x-6 gap-y-10 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 xl:gap-x-8 py-4">
        {courses.map((product) => (
          <Link
            key={product.id}
